...
const semesterOrder: Record<SemesterName, number> = {
  Alpha: 1,
  Omega: 2,
};

function isFutureTerm(term: TermRow) {
  if (term.level > studentProgress.currentLevel) {
    return true;
  }
...
function GaugeMeter({
  ...
  const color = getDialColor(clampedValue);
    ...
    >
      <div className="relative w-full max-w-[320px] aspect-[320/260] mx-auto flex flex-col">
        <svg viewBox="0 0 320 220" className="w-full h-auto drop-shadow-sm">
          ...
          {/* Needle - base anchored at centre (160,160), tip sweeps the arc */}
          {isMounted ? (
            <motion.g
              style={{
            ...
            >
              <polygon
                points="154,160 154,88 160,72 166,88 166,160"
                fill="#dc2626"
                filter={`url(#shadow-${containerId})`}
              />
            </motion.g>
...
export default function ResultsPage() {
  ...
  // Added to track the active term for the gauges calculation
  ...
  const toggleRow = (term: TermRow) => {
    ...
    // Set clicked row as the selected term ID for the gauges above!
  ...
  const handleViewPreviousResults = () => {
    console.log("View previous results", { studentProgress });
  };

  const handlePrintTranscript = () => {
    console.log("Print unofficial transcript", { studentProgress });
  };

  // Dynamically calculate the stats based on the selected term from termRows
  // assuming terms are chronologically reverse ordered in `termRows`.
  const stats = useMemo(() => {
    ...
    const termGpa = selectedTerm.gpa || 0;
